...
@MainActor
@Test
func appModelPresentsAndAcceptsDisclosureWithoutStartingWork() async throws {
    let driver = SettingsAppTestDriver(
        snapshot: try SettingsAppTestFactory.snapshot(
    ...
    await model.reviewDeepDiveDisclosure()
    #expect(model.deepDiveDisclosurePresentation == .reviewAgain)

    await model.submitDeepDiveDisclosure(.accept)
    #expect(model.deepDiveDisclosurePresentation == nil)
    #expect(
        model.settingsState.snapshot?.disclosureReadiness == .accepted
    )
    #expect(await driver.disclosureDecisions == [.accept])
    #expect(await driver.investigationStartCount == 0)
}

@MainActor
@Test
func declineAndForgetAcceptanceRemainTypedSettingsMutations() async throws {
    let driver = SettingsAppTestDriver(
        snapshot: try SettingsAppTestFactory.snapshot(
            disclosureReadiness: .notPresented
        )
    ...
    await model.reviewDeepDiveDisclosure()
    await model.submitDeepDiveDisclosure(.notNow)
    #expect(model.settingsState.snapshot?.disclosureReadiness == .declined)
    #expect(await driver.disclosureDecisions == [.notNow])

    await model.forgetDeepDiveDisclosureAcceptance()
    #expect(
        model.settingsState.snapshot?.disclosureReadiness == .notPresented
    )
    #expect(await driver.forgetDisclosureCount == 1)
    #expect(await driver.investigationStartCount == 0)
}

@MainActor
@Test
func disclosurePersistenceFailureDoesNotPromoteConsent() async throws {
    let driver = SettingsAppTestDriver(
        snapshot: try SettingsAppTestFactory.snapshot(
            disclosureReadiness: .notPresented
        ),
        disclosureResult: .persistenceFailed
    )
    let model = settingsAppModel(driver: driver)
    await model.refreshSettings()
    ...
    #expect(
        model.settingsState.reasonKey?.rawValue
            == "settings.error.disclosurePersistenceFailed"
    )
    #expect(await driver.investigationStartCount == 0)
}

@Test
func liveMemoryDependenciesPersistDisclosureWithoutRuntimeAdmission()
    async throws
{
    ...
    #expect(
        await dependencies.evaluateDeepDiveFirstUse(
            .settingsReview,
            dimensions
        ) == .presentDisclosure(.reviewAgain)
    )
    #expect(
        await dependencies.submitDeepDiveDisclosure(
            .accept,
            SettingsAppTestFactory.now
        ) == .acceptedRequiresFreshAdmission
    )
    let snapshot = try await dependencies.loadSettings()
    #expect(snapshot.disclosureReadiness == .accepted)
    #expect(snapshot.runtimeEvidence == .unverified)
    #expect(snapshot.dependencyReadiness == .task38FacadeUnavailable)
}
...
private func settingsDependencies(
    ...
    AppDependencies(
        ...
        loadSettings: { await driver.load() },
        evaluateDeepDiveFirstUse: { request, dimensions in
            await driver.evaluateDisclosure(request, dimensions: dimensions)
        },
        submitDeepDiveDisclosure: { decision, decidedAt in
            await driver.submitDisclosure(decision, decidedAt: decidedAt)
        },
        forgetDeepDiveDisclosureAcceptance: {
            await driver.forgetDisclosure()
        },
...
private actor SettingsAppTestDriver {
    ...
    private(set) var disclosureDecisions: [DeepDiveFirstUseDecision] = []
    private(set) var forgetDisclosureCount = 0
    private(set) var investigationStartCount = 0
    private let disclosureResult: DeepDiveFirstUseDecisionResult

    init(
        snapshot: SettingsSnapshot,
        disclosureResult: DeepDiveFirstUseDecisionResult =
            .acceptedRequiresFreshAdmission
    ) {
        self.snapshot = snapshot
        self.disclosureResult = disclosureResult
    }
    ...
    func evaluateDisclosure(
        _ request: DeepDiveFirstUseRequest,
        dimensions: DeepDiveAdmissionDimensions
    ) -> DeepDiveFirstUseEvaluation {
        guard request == .settingsReview else { return .notApplicable }
        return .presentDisclosure(
            dimensions.disclosure == .obsolete ? .obsolete : .reviewAgain
        )
    }
    ...
    func submitDisclosure(
        ...
        disclosureDecisions.append(decision)
        guard disclosureResult != .persistenceFailed else {
    ...
    func forgetDisclosure() -> DeepDiveForgetAcceptanceResult {
        forgetDisclosureCount += 1
        snapshot = snapshot.replacing(
            disclosureReadiness: .notPresented
        )
        return .forgotten
