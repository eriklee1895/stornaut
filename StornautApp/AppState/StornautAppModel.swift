...
final class StornautAppModel {
    ...
    private(set) var deepDiveDisclosurePresentation:
    ...
    var deepDiveWorkflowReadiness: DeepDiveWorkflowReadiness {
        if scanState.isActive || scanTask != nil {
            return .quickScanRunning
        }
        if reviewExecutionTask != nil || cleanupResultTask != nil {
            return .cleanupExecuting
        }
        if historyDeleteIsActive {
            return .historyMutationInProgress
        }
        if settingsMutationIsActive {
            return .settingsMutationInProgress
        }
        return .idle
    }

    var deepDiveSourceReadiness: DeepDiveSourceReadiness? {
        switch pageState.phase {
        case .stale:
            return .stale
        case .empty, .loading, .error:
            return pageState.projection == nil ? .missing : nil
        case .partial, .cancelled, .limitedPermission, .success:
            return nil
        }
    }
    ...
    init(
        ...
        deepDiveDisclosurePresentation = nil
    ...
    func reviewDeepDiveDisclosure() async {
        guard let snapshot = settingsState.snapshot else {
            return
        }
        let model = SettingsModel(
            state: .loaded(snapshot),
            latestProjection: pageState.projection,
            workflowReadiness: settingsMutationIsActive
                ? .settingsMutationInProgress
                : .idle
        )
        switch await dependencies.evaluateDeepDiveFirstUse(
            .settingsReview,
            model.codex.deepDive.dimensions
        ) {
        case let .presentDisclosure(reason):
            deepDiveDisclosurePresentation = reason
        case .blocked, .readyRequiresFreshAdmission,
             .operationInProgress, .notApplicable:
            break
        }
    }

    func submitDeepDiveDisclosure(
        _ decision: DeepDiveFirstUseDecision
    ) async {
        guard deepDiveDisclosurePresentation != nil,
              beginSettingsMutation(.disclosure)
        else {
            return
        }
        let previous = settingsState.snapshot
        defer { settingsMutationIsActive = false }
        let result = await dependencies.submitDeepDiveDisclosure(
            decision,
            now()
        )
        switch result {
        case .acceptedRequiresFreshAdmission, .declined:
            let persistedReadiness: DeepDiveDisclosureReadiness =
                result == .acceptedRequiresFreshAdmission
                    ? .accepted
                    : .declined
            let persisted = previous?.replacing(
                disclosureReadiness: persistedReadiness,
                refreshedAt: now()
            )
            deepDiveDisclosurePresentation = nil
            do {
                let refreshed = try await dependencies.loadSettings()
                settingsState = .loaded(refreshed)
            } catch {
                settingsState = .failed(
                    snapshot: persisted ?? previous,
                    reasonKey: token(
        ...
        case .presentationRequired, .operationInProgress:
            settingsState = previous.map(SettingsState.loaded) ?? .idle
        }
    }

    func forgetDeepDiveDisclosureAcceptance() async {
        guard beginSettingsMutation(.disclosure) else { return }
        let previous = settingsState.snapshot
        defer { settingsMutationIsActive = false }
        let result = await dependencies.forgetDeepDiveDisclosureAcceptance()
        switch result {
        case .forgotten:
            let forgotten = previous?.replacing(
                disclosureReadiness: .notPresented,
                refreshedAt: now()
            )
            do {
                settingsState = .loaded(try await dependencies.loadSettings())
            } catch {
                settingsState = .failed(
                    snapshot: forgotten ?? previous,
                    reasonKey: token(
                        "settings.error.disclosurePersistenceFailed"
                    )
                )
            }
        case .operationInProgress, .persistenceFailed:
            settingsState = .failed(
                snapshot: previous,
                reasonKey: token(
                    "settings.error.disclosurePersistenceFailed"
