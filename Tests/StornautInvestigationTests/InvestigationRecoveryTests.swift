...
struct InvestigationRecoveryTests {
    ...
    func userStopInterruptsOnceThenCommitsVerifiedPartial() async throws {
        ...
        let fallbackEvidence = try #require(
            fixture.store.terminalCommands[0].report?.evidence
        )
        #expect(fallbackEvidence.count == fixture.plan.targets.count)
        #expect(fallbackEvidence.allSatisfy { record in
