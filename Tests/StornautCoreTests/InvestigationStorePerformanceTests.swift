...
@Test
func investigationReportProjectionHonorsStoreDeadline_BitsUT() async throws {
    let fixture = try InvestigationStoreV4Fixture()
    let deadline = InvestigationMonotonicTestClock()
    ...
    let session = try await store.createInvestigation(
        fixture.command(
            investigationID: "investigation-report-deadline",
            runID: "investigation-run-report-deadline"
        )
    )
    deadline.armDeadline()

    await #expect(throws: EvidenceStoreError.operationDeadlineExceeded) {
        _ = try await store.investigationReportReadSnapshot(
            investigationID: session.id,
            runID: session.runID,
            reportID: InvestigationReportID(
                rawValue: "investigation-report-deadline"
            )!
        )
    }
    #expect(await store.storeHealth() == .ready)
