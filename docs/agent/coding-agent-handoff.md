...
与 ii-c-b2a2、resolved-lineage L1/L2 已完成/non-admitting，L2 final review none；历史 pre-v8 顺序为 ii-c-c → L3c3d → L3c4；v8–v13 均已消费失败且不可重试；v13 为 closed post-arm failure，persistent Gate residue 保留；authorization hardening 已完成/non-admitting；fresh v14 已基于 `d5a7df3` 授权，authorization-only record `959ac3a` 已推送，pending/unconsumed；当前 frontier 为 v14 campaign → L3c3d → L3c4；这些 repairs 是 machine-campaign prerequisite
