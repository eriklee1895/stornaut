...
> **Status:** Final production admission pending under the
> [three-day sprint](README.md#三天完整收尾冲刺2026-09-18-至-2026-09-20).
> Test/acceptance preparation may start now; admission requires reviewed and
> integrated Tasks 39–43. The normal factory is implemented with Task 42 rather
> than first assembled here. Phase D and E/F may share the same final full only
> on the exact same integrated tree with all applicable checks included.
>
> **Parent plan:**
...
Task 44 does not own Adapters, real Registered Actions or ordinary-App cleanup:
the sprint's A work package delivers them separately and joins final acceptance.
They are not dropped from complete v1. Background work, telemetry and arbitrary
provider selection remain excluded; release distribution, Developer ID and
notarization retain separate authorization.

## 2. Preconditions

The final integration must contain committed and pushed Tasks 35–43 with:

- zero unresolved P0–P2 review findings;
- retained Task 35–39 checkpoint evidence, including Task 39's final full;
- scoped tests and actual-App evidence for Tasks 40–43 under the sprint tiers;
- one current integrated final full, required before Task 44 admission rather
  than separately repeated for each product-only Task;
- fresh canonical sprint state and normative document links;
- the final commit matching its intended pushed remote baseline.
...
Task 42 implements the separate normal-product composition root behind the
closed gate; Task 44 revalidates and admits it rather than first assembling it.
It constructs a new instance of the closed Task 38 facade from reviewed low-level
...
git diff --check
# The final full owns its regression; do not repeat a separate serial here.
scripts/verify --full
```
