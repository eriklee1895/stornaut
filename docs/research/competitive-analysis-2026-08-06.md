...
> 配套文档：[PRD](../product/PRD.md)、[技术架构](../architecture/system-architecture.md)
...
对“个人自用 + 开源”目标而言，这个项目仍然非常值得做。它不需要在商业功能数量上超过所有竞品；它需要把匿名化调研中有效的 Agent 调查方法，变成一个透明、可审计、自己敢长期使用的系统。
...
匿名化调研中，HOME 与已用空间曾存在显著差额，最终由 VM/swap、系统目录、根隐藏目录等解释。Stornaut 可以把以下内容做成统一账本：
...
为了让 Stornaut 的价值可验证，可使用匿名化目录 fixture，然后用同一组任务比较：
