...
- `overview-round2-a-orbit-ledger-dark.webp` — A / Orbit Ledger: visual orbit plus readable two-axis ledger; recommended balance.
- `overview-round2-b-constellation-dark.webp` — B / Constellation: historical exploration asset. Its star/constellation metaphor is rejected; retain only direct labels and functional Probe placement.
- `overview-round2-c-native-console-dark.webp` — C / Native Console: compact professional instrument workspace; highest information efficiency.
...
- `overview-canonical-dark.webp` — approved-direction dark reference.
- `overview-canonical-light.webp` — theme-paired light reference with identical hierarchy.
...
- `deep-round2-a-probe-focus-dark.webp` — A / Probe Focus: strongest active-observatory feeling; orbit and live findings share attention.
- `deep-round2-b-guided-journey-dark.webp` — B / Guided Journey: process-first and easiest to understand; recommended default.
- `deep-round2-c-evidence-inspector-dark.webp` — C / Evidence Inspector: optional audit state with the right-side Inspector open, not the default screen.
...
- `review-round2-a-decision-table-dark.webp` — A / Decision Table: native grouped outline/table, maximum comparison efficiency; recommended default.
- `review-round2-b-decision-cards-dark.webp` — B / Decision Cards: more explanatory expandable groups, lower density.
- `review-round2-c-evidence-inspector-dark.webp` — C / Evidence Inspector: A with an unselected Review Recommended row focused and the optional audit Inspector open.
...
- `review-canonical-dark.webp` — approved-direction dark default Review reference.
- `review-canonical-light.webp` — theme-paired light default Review reference with identical hierarchy and safety state.
- `review-round2-c-evidence-inspector-dark.webp` — approved Inspector-state reference; the focused Review Recommended row remains unchecked and the Inspector contains no execution action.

## Quick Scan progress — five-candidate internal draw

See [round notes](QUICK-SCAN-ROUND-1.md).

- `quick-scan-round1-a-stage-rail-dark.webp` — strongest process explanation.
- `quick-scan-round1-b-growing-ledger-dark.webp` — strongest measured/unmeasured visualization.
- `quick-scan-round1-c-native-operations-dark.webp` — highest observability and density.
- `quick-scan-round1-d-focus-discoveries-dark.webp` — lowest-text visual candidate; rejected semantic drift.
- `quick-scan-round1-e-results-taking-shape-dark.webp` — strongest progress-to-results continuity.
...
- `quick-scan-canonical-dark.webp` — approved dark reference.
- `quick-scan-canonical-light.webp` — theme-paired light reference.

## Scan Results — five-candidate internal draw

See [round notes](SCAN-RESULTS-ROUND-1.md).

- `scan-results-round1-a-lifecycle-outline-dark.webp` — grouped lifecycle outline; recommended default.
- `scan-results-round1-b-opportunity-first-dark.webp` — opportunity cards above a flat table.
- `scan-results-round1-c-category-navigator-dark.webp` — category browser with a secondary navigator.
- `scan-results-round1-d-evidence-inspector-dark.webp` — optional read-only Inspector state.
- `scan-results-round1-e-ledger-results-dark.webp` — two-axis summary ledger plus dense table.
...
- `scan-results-canonical-dark.webp` — approved dark default Results reference.
- `scan-results-canonical-light.webp` — theme-paired light default Results reference.
- `scan-results-round1-d-evidence-inspector-dark.webp` — approved Inspector behavior reference subject to the documented badge/color corrections.

## Onboarding and permissions — five-candidate internal draw

See [round notes](ONBOARDING-ROUND-1.md).

- `onboarding-round1-a-guided-focus-dark.webp` — one decision per page with a persistent three-step rail; approved base.
- `onboarding-round1-b-setup-checklist-dark.webp` — useful future Settings status pattern, rejected for first launch.
- `onboarding-round1-c-coverage-compare-dark.webp` — strongest Full/Limited transparency; compressed into the approved step 2.
- `onboarding-round1-d-contextual-preview-dark.webp` — educational coverage example, reserved for help/details.
- `onboarding-round1-e-native-sheet-dark.webp` — compact sheet, rejected as the three-step setup structure.
...
- `onboarding-welcome-canonical-dark.webp` / `onboarding-welcome-canonical-light.webp` — step 1 theme pair.
- `onboarding-full-disk-access-canonical-dark.webp` / `onboarding-full-disk-access-canonical-light.webp` — step 2 theme pair.
- `onboarding-connect-codex-canonical-dark.webp` / `onboarding-connect-codex-canonical-light.webp` — step 3 theme pair.

## Cleanup Result — five-candidate internal draw

See [round notes](CLEANUP-RESULT-ROUND-1.md).

- `cleanup-result-round1-a-accounting-ledger-dark.webp` — strongest explicit accounting separation; adopted as the numeric contract.
- `cleanup-result-round1-b-reversible-first-dark.webp` — clearest recoverability-first hierarchy; approved default base.
- `cleanup-result-round1-c-plan-vs-actual-dark.webp` — adopted for collapsed `Accounting Details`.
- `cleanup-result-round1-d-manifest-timeline-dark.webp` — adopted for the separate Manifest audit view.
- `cleanup-result-round1-e-partial-outcome-dark.webp` — approved partial/failure-state reference.
...
- `cleanup-result-canonical-dark.webp` — approved dark successful-result reference.
- `cleanup-result-canonical-light.webp` — theme-paired light successful-result reference with identical hierarchy and accounting semantics.

## History — five-candidate internal draw

See [round notes](HISTORY-ROUND-1.md).

- `history-round1-a-unified-timeline-dark.webp` — strongest chronology and retention language; grouping adopted.
- `history-round1-b-audit-ledger-dark.webp` — expert audit table; retained as a possible dense/table substate, not default.
- `history-round1-c-change-story-dark.webp` — approved optional `Storage Trend` direction with explicit non-causality caption.
- `history-round1-d-session-cards-dark.webp` — approachable but too low-density; rejected as desktop default.
- `history-round1-e-master-detail-dark.webp` — strongest native macOS audit workspace; approved default base after TTL corrections.
...
- `history-canonical-dark.webp` — approved dark default History reference.
- `history-canonical-light.webp` — theme-paired light default History reference with identical selection, retention, accounting, and action semantics.

## Settings — five-candidate internal draw

See [round notes](SETTINGS-ROUND-1.md).

- `settings-round1-a-sidebar-privacy-dark.webp` — native six-section sidebar and fixed Privacy & Data policy rows; approved shell.
- `settings-round1-b-toolbar-permissions-dark.webp` — classic toolbar-tabs exploration; retained as Permissions content reference, not the shell.
- `settings-round1-c-status-summary-general-dark.webp` — compact General + Setup Status; adopted after fixture/control corrections.
- `settings-round1-d-single-scroll-dark.webp` — single-scroll form; rejected because it compresses unrelated preference and safety concepts.
- `settings-round1-e-local-knowledge-dark.webp` — structured knowledge management; adopted after conservative example corrections.
...
- `settings-general-canonical-dark.webp` / `settings-general-canonical-light.webp` — General, appearance/language, setup status, and explicit on-demand behavior.
- `settings-codex-deep-dive-canonical-dark.webp` / `settings-codex-deep-dive-canonical-light.webp` — independent Codex installation and Deep Dive safety states plus default budget presets.
- `settings-local-knowledge-canonical-dark.webp` / `settings-local-knowledge-canonical-light.webp` — structured confirmed facts, provenance, review, and forgetting.

## Resilience states — cross-flow draw

See [round notes and behavior matrix](RESILIENCE-STATES-ROUND-1.md).

- `resilience-limited-coverage-canonical-dark.webp` / `resilience-limited-coverage-canonical-light.webp` — measured Quick Scan results remain useful while inaccessible scope is explicitly unmeasurable.
- `resilience-deep-dive-safety-blocked-canonical-dark.webp` / `resilience-deep-dive-safety-blocked-canonical-light.webp` — Deep Dive remains paused when full investigation capabilities are unavailable or write/no-Executor isolation fails; Quick Scan remains available and no write bypass exists.
- `resilience-stale-plan-canonical-dark.webp` / `resilience-stale-plan-canonical-light.webp` — execution preflight blocks a changed plan and requires affected items to be refreshed.
- `resilience-partial-investigation-canonical-dark.webp` / `resilience-partial-investigation-canonical-light.webp` — verified partial evidence is preserved when the budget ends; unresolved targets remain Unknown.
- `resilience-history-expired-canonical-dark.webp` / `resilience-history-expired-canonical-light.webp` — expired linked evidence does not erase the minimum Cleanup Manifest; corrupt history is isolated by row.
- `resilience-deep-dive-safety-blocked-draft-dark.webp` — rejected audit draft because stale success metrics contradicted a not-started investigation.
