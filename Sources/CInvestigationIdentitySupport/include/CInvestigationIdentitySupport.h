...
typedef struct {
    uid_t audit_user_id;
    int32_t audit_session_id;
} stornaut_investigation_current_audit_identity;
...
/*
 * Cross-UID observer for a process whose PROC_PIDTBSDINFO and per-PID BSM
 * surfaces may both be denied. Reads only two KERN_PROC_PID snapshots around
 * getsid(2) and never acquires a task port or queries child audit state. The
 * result intentionally has no audit fields.
 */
int stornaut_investigation_cross_uid_process_snapshot_for_pid(
    pid_t process_id,
    stornaut_investigation_cross_uid_process_snapshot *snapshot
);

/* Reads only the calling process's inherited audit user/session anchor. */
int stornaut_investigation_current_audit_identity_read(
