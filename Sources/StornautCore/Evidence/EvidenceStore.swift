...
public actor EvidenceStore {
    ...
    func investigationDegradations(
        investigationID: InvestigationID,
        reportID: InvestigationReportID,
        limit: Int,
        offset: Int
    ) throws -> StorePage<InvestigationStoredDegradation> {
        try validatePage(limit: limit, offset: offset)
        let rows = try connection.query(
            """
            SELECT report_id, run_id, degradation_id, ordinal,
                   degradation_kind, payload
            FROM investigation_report_degradations
            WHERE investigation_id = ? AND report_id = ?
            ORDER BY ordinal ASC, degradation_id ASC
            LIMIT ? OFFSET ?
            """,
            bindings: [
                .text(investigationID.rawValue),
            ...
            operation: "investigation.degradation.page"
        ) { statement in
            (
                reportID: columnText(statement, 0),
                runID: columnText(statement, 1),
                degradationID: columnText(statement, 2),
                ordinal: sqlite3_column_int64(statement, 3),
                kind: columnText(statement, 4),
                payload: columnText(statement, 5)
            )
        }
        var records: [InvestigationStoredDegradation] = []
        var corrupt: [String] = []
        for row in rows {
            do {
                let record = try InvestigationStoredDegradation(
                    investigationID: investigationID,
                    reportID: InvestigationReportID(
                        validating: row.reportID
                    ),
                    ...
                    id: InvestigationDegradationID(
                        validating: row.degradationID
                    ),
                    ordinal: checkedInvestigationOrdinal(row.ordinal),
                    kind: investigationEnum(
                        InvestigationPersistedDegradationKind.self,
                        rawValue: row.kind
                    ),
                    payload: decodeInvestigationPayload(
                        InvestigationDegradationPayload.self,
                        text: row.payload
                    )
                )
                guard record.reportID == reportID else {
                ...
                corrupt.append(row.degradationID)
            }
        }
        return StorePage(records: records, corruptRecordIDs: corrupt)
    }
    ...
    func investigationReportReadSnapshot(
        ...
        runID: InvestigationRunID,
        reportID: InvestigationReportID
    ) throws -> InvestigationReportReadSnapshot {
        try connection.transaction(operation: "investigation.reportProjection") {
            let status = try rejoinInvestigationInCurrentTransaction(
                id: investigationID,
                currentTime: now()
            )
            guard let run = try loadInvestigationRun(
                investigationID: investigationID,
                runID: runID
            ), run.reportID == reportID,
               let report = try loadInvestigationReport(
                    investigationID: investigationID,
                    reportID: reportID
               ), report.runID == runID
            else {
                return InvestigationReportReadSnapshot(
            ...
            let evidence = try allInvestigationEvidence(
                investigationID: investigationID,
                reportID: reportID
            )
            let degradations = try allInvestigationDegradations(
                investigationID: investigationID,
                reportID: reportID
            )
            let budget = try allInvestigationBudgetEvents(
                investigationID: investigationID,
                runID: runID
            )
            return InvestigationReportReadSnapshot(
                sourceStatus: status,
                plan: run.plan,
                terminalCause: run.terminalCause,
                report: report,
                evidence: evidence.records,
                degradations: degradations.records,
                budgetEvents: budget.records,
                corruptRecordIDs: Set(
                    evidence.corruptRecordIDs
                        + degradations.corruptRecordIDs
                        + budget.corruptRecordIDs
                ).sorted()
            )
        }
    }

    private func allInvestigationEvidence(
        investigationID: InvestigationID,
        reportID: InvestigationReportID
    ) throws -> StorePage<InvestigationStoredEvidence> {
        var records: [InvestigationStoredEvidence] = []
        ...
        var offset = 0
        while true {
            let page = try investigationEvidence(
                investigationID: investigationID,
                reportID: reportID,
            ...
            let physicalCount = page.records.count + page.corruptRecordIDs.count
            guard physicalCount == maximumStorePageSize else { break }
            offset += physicalCount
        }
        return StorePage(records: records, corruptRecordIDs: corrupt)
    }

    private func allInvestigationDegradations(
        investigationID: InvestigationID,
        reportID: InvestigationReportID
    ) throws -> StorePage<InvestigationStoredDegradation> {
        var records: [InvestigationStoredDegradation] = []
        ...
        var offset = 0
        while true {
            let page = try investigationDegradations(
                investigationID: investigationID,
                reportID: reportID,
                limit: maximumStorePageSize,
                offset: offset
    ...
    private func allInvestigationBudgetEvents(
        investigationID: InvestigationID,
        runID: InvestigationRunID
    ) throws -> StorePage<InvestigationStoredBudgetEvent> {
        var records: [InvestigationStoredBudgetEvent] = []
        ...
        var offset = 0
        while true {
            let page = try investigationBudgetEvents(
                investigationID: investigationID,
                runID: runID,
                limit: maximumStorePageSize,
                offset: offset
    ...
    public func saveCleanupPlan(_ plan: CleanupPlan) throws {
        try connection.transaction(operation: "plan.save") {
            try saveCleanupPlanInCurrentTransaction(plan)
        }
    }

    private func saveCleanupPlanInCurrentTransaction(
        _ plan: CleanupPlan
    ) throws {
        ...
        try insertImmutableExpiringRecordInCurrentTransaction(
            table: "cleanup_plans",
            id: plan.id.rawValue,
    ...
    func commitCleanupPlanForInvestigation(
        investigationID: InvestigationID,
        runID: InvestigationRunID,
        reportID: InvestigationReportID,
        plan: CleanupPlan?
    ) throws -> InvestigationRejoinResult {
        try connection.transaction(operation: "investigation.cleanupPlanJoin") {
            let status = try rejoinInvestigationInCurrentTransaction(
                id: investigationID,
                currentTime: now()
            )
            guard status == .matching else { return status }
            try connection.checkOperationControl()
            guard let current = try loadInvestigation(
                id: investigationID
            ), current.runID == runID,
                let run = try loadInvestigationRun(
                    investigationID: investigationID,
                    runID: runID
                ),
                run.reportID == reportID,
                try loadInvestigationReport(
                    investigationID: investigationID,
                    reportID: reportID
                )?.runID == runID
            else {
            ...
            if let plan {
                let admittedBindings = Set(current.plan.targets.compactMap { target in
                    switch target.sourceBinding {
                    case let .classification(classificationID, snapshotID):
                        return "\(snapshotID.rawValue):\(classificationID.rawValue)"
                    case .snapshot, .spaceLedger:
                        return nil
                    }
                })
                guard let identity = try investigationSourceIdentity(
                    id: investigationID
                ),
                    plan.scanSessionID == identity.scanSessionID,
                    plan.scanScopeID == identity.scanScopeID,
                    plan.items.allSatisfy({ item in
                        admittedBindings.contains(
                            "\(item.snapshotID.rawValue):\(item.classificationID.rawValue)"
                        )
                    })
                else {
                    throw InvestigationPersistenceError.invalidCommand
                ...
                try saveCleanupPlanInCurrentTransaction(plan)
                connection.reportInvestigationProgress(
                    phase: .verification,
                    force: true
                )
                try connection.checkOperationControl()
    ...
    private func insertImmutableExpiringRecord(
        ...
        try connection.transaction(operation: operation) {
            try insertImmutableExpiringRecordInCurrentTransaction(
                table: table, id: id, parentID: parentID,
                parentColumn: parentColumn, timestampColumn: timestampColumn,
    ...
    private func insertImmutableExpiringRecordInCurrentTransaction(
        table: String,
        id: String,
        parentID: String,
        parentColumn: String,
