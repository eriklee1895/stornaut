...
public struct CleanupPlanBuilder: Sendable {
    ...
    public func build(
        ...
        do {
            return try await buildValidated(
                ...
                persistPlan: true,
                investigationTargetBindings: nil
            )
    ...
    func buildForInvestigation(
        sessionID: ScanSessionID,
        targetBindings: [InvestigationReportTargetBinding]
    ) async -> CleanupPlanBuildOutcome {
        do {
            guard let session = try await store.scanSession(id: sessionID),
                  session.completedScopes.count == 1,
                  let rootPath = session.completedScopes.first?.rootPath
            else {
                return .scanAgain([
            ...
            return try await buildValidated(
                sessionID: sessionID,
                rootURL: URL(filePath: rootPath.rawValue),
                persistPlan: false,
                investigationTargetBindings: targetBindings
            )
        } catch is CancellationError {
            return .unavailable([token("review.unavailable.cancelled")])
        } catch CleanupPlanBuildError.corruptTruth {
            return .scanAgain([token("review.scan-again.corrupt-truth")])
        } catch {
            return .unavailable([
                token("review.unavailable.persisted-truth"),
            ])
    ...
    private func buildValidated(
        ...
        rootURL: URL,
        persistPlan: Bool,
        investigationTargetBindings: [InvestigationReportTargetBinding]?
        ...
        while true {
            ...
            for record in page.records {
                ...
                if let investigationTargetBindings,
                   !investigationTargetBindings.contains(where: { target in
                       guard target.snapshotID == record.snapshot.id else {
                           return false
                       }
                       return target.classificationID == nil
                           || target.classificationID == record.classification.id
                   })
                {
                    continue
                }
